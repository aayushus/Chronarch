...
export default function KioskSettings() {
  ...
  if (loading) {
    return <div style={{ padding: "32px 0", color: "var(--text-tertiary)", fontSize: "var(--text-md)" }}>Loading wall displays…</div>;
  }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            ...
            >
              ...
              {canManage && editingId === d.id && (
                <div style={{ position: "absolute", inset: 8, zIndex: 2, background: "var(--bg-raised)", display: "grid", gridTemplateColumns: "repeat(5, minmax(90px, 1fr)) auto", gap: 8, alignItems: "end", padding: 10, borderRadius: "var(--radius-md)", border: "1px solid var(--border-subtle)" }}>
                  {(["name", "location_label", "sleep_start", "sleep_end"] as const).map((key) => <label key={key} style={{ fontSize: "var(--text-sm)", color: "var(--text-secondary)" }}>{key.replace("_", " ")}<input className="input-standard" value={edit[key]} onChange={(e) => setEdit({ ...edit, [key]: e.target.value })} style={{ width: "100%", marginTop: 3, fontSize: "var(--text-sm)" }} /></label>)}
                  <label style={{ fontSize: "var(--text-sm)", color: "var(--text-secondary)" }}>screensaver (sec)<input className="input-standard" type="number" min={10} max={3600} value={edit.screensaver_timeout_seconds} onChange={(e) => setEdit({ ...edit, screensaver_timeout_seconds: Number(e.target.value) })} style={{ width: "100%", marginTop: 3, fontSize: "var(--text-sm)" }} /></label>
              ...
              {canManage && (
                <>
                  <button onClick={() => beginEdit(d)} className="btn-secondary hoverable" style={{ padding: "7px 14px", fontSize: "var(--text-sm)", whiteSpace: "nowrap" }}>Edit</button>
                  <button
      ...
      {canManage && (
        ...
        >
          ...
          >
            <div>
              <div style={{ fontSize: "var(--text-lg)", fontWeight: 700, color: "var(--text-primary)" }}>Pair with code</div>
              <p style={{ fontSize: "var(--text-md)", color: "var(--text-secondary)", margin: "6px 0 0", lineHeight: 1.5 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 12 }}>
                <a
                  ...
                  style={{ flex: 1, minWidth: 0, padding: "9px 10px", fontSize: "var(--text-sm)", color: "var(--accent)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", textDecoration: "none" }}
                  title={pairUrl}
                >
                  {pairUrl}
                </a>
                <button
                  ...
                  style={{ flexShrink: 0, padding: "9px 11px", fontSize: "var(--text-sm)", fontWeight: 600 }}
                >
                  Copy link
                </button>
              </div>
              <a href={pairUrl} target="_blank" rel="noreferrer" className="btn-secondary hoverable" style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6, alignSelf: "flex-start", marginTop: 10, padding: "8px 12px", fontSize: "var(--text-sm)", fontWeight: 600, textDecoration: "none" }}>
                Open pairing page
          ...
          >
            <div>
              <div style={{ fontSize: "var(--text-lg)", fontWeight: 700, color: "var(--text-primary)" }}>Pair manually</div>
              <p style={{ fontSize: "var(--text-md)", color: "var(--text-secondary)", margin: "6px 0 0", lineHeight: 1.5 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div>
                <label htmlFor="kiosk-name" style={{ display: "block", fontSize: "var(--text-sm)", fontWeight: 600, marginBottom: 6, color: "var(--text-primary)" }}>
                  Name
                </label>
                <input
                  id="kiosk-name"
                  ...
                  className="input-standard"
                  style={{ width: "100%", fontSize: "var(--text-md)", padding: "8px 12px" }}
                />
              </div>

              <div>
                <label htmlFor="kiosk-location" style={{ display: "block", fontSize: "var(--text-sm)", fontWeight: 600, marginBottom: 6, color: "var(--text-primary)" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div>
                  <label htmlFor="kiosk-sleep-start" style={{ display: "block", fontSize: "var(--text-sm)", fontWeight: 600, marginBottom: 6, color: "var(--text-primary)" }}>
                    Screen sleeps at
                  </label>
                  <input className="input-standard input-native"
                ...
                <div>
                  <label htmlFor="kiosk-sleep-end" style={{ display: "block", fontSize: "var(--text-sm)", fontWeight: 600, marginBottom: 6, color: "var(--text-primary)" }}>
                    Wakes at
                  </label>
                  <input className="input-standard input-native"
                    id="kiosk-sleep-end"
              ...
              <div>
                <label htmlFor="kiosk-screensaver-timeout" style={{ display: "block", fontSize: "var(--text-sm)", fontWeight: 600, marginBottom: 6, color: "var(--text-primary)" }}>
                  Screensaver countdown <span style={{ fontWeight: 400, color: "var(--text-tertiary)" }}>(seconds of inactivity)</span>
                </label>
                <input id="kiosk-screensaver-timeout" type="number" min={10} max={3600} step={10} value={screensaverTimeout} onChange={(e) => setScreensaverTimeout(Math.max(10, Math.min(3600, Number(e.target.value) || 30)))} className="input-standard" style={{ width: "100%", fontSize: "var(--text-md)", padding: "8px 12px" }} />
              </div>
            </div>
