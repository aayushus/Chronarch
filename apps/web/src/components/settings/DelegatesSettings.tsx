...
function Avatar({ email, tint }: { email: string; tint: "accent" | "violet" }) {
  ...
  const fg = tint === "accent" ? "var(--accent)" : "var(--accent-alt)";
  return (
    <span
      aria-hidden
      style={{
...
function StatusPill({ active }: { active: boolean }) {
  return (
    <span
      style={{
        fontSize: "var(--text-sm)", fontWeight: 600, padding: "2px 9px", borderRadius: "var(--radius-lg)", whiteSpace: "nowrap",
...
function Switch({ checked, onChange, label }: { checked: boolean; onChange: () => void; label: string }) {
  return (
    <button
      ...
      className="hoverable"
      style={{
        width: 34, height: 20, borderRadius: "var(--radius-lg)", border: "none", cursor: "pointer", padding: 2,
    ...
    >
      <span style={{ width: 16, height: 16, borderRadius: "50%", background: "var(--text-on-fill)" }} />
    </button>
  );
}
...
export default function DelegatesSettings() {
  ...
  if (loading) {
    return <div style={{ padding: "32px 0", color: "var(--text-tertiary)", fontSize: "var(--text-md)" }}>Loading pairings…</div>;
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 20 }}>
        ...
        <SummaryCard icon="calendar" tint="var(--accent-alt-soft)" color="var(--accent-alt)" label="Calendars granted" value={grantedCount} />
      </div>
                ...
                <div style={{ padding: "14px 18px", display: "flex", alignItems: "center", gap: 14 }}>
                  ...
                  >
                    ...
                    <span style={{ minWidth: 0 }}>
                      <span style={{ fontSize: "var(--text-md)", fontWeight: 600, display: "block", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        {d.owner_email} <span style={{ color: "var(--text-tertiary)", fontWeight: 400 }}>· assisted by {d.delegate_email}</span>
                      </span>
                      <span style={{ fontSize: "var(--text-sm)", color: "var(--text-secondary)", marginTop: 2, display: "block" }}>
                  ...
                  <button
                    ...
                    title="Stop sharing"
                    style={{ background: "none", border: "none", borderRadius: "var(--radius-sm)", color: "var(--text-tertiary)", cursor: "pointer", padding: 6, display: "inline-flex" }}
                  >
                        ...
                        return (
                          ...
                          >
                            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                              ...
                              <span style={{ fontSize: "var(--text-md)", fontWeight: 600, flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                                {cal.name}
                              </span>
                              {cal.writable === false && (
                                <span style={{ fontSize: "var(--text-xs)", fontWeight: 600, color: "var(--text-tertiary)", background: "var(--bg-raised)", padding: "2px 7px", borderRadius: "var(--radius-sm)" }}>
                                  Read-only
                                </span>
                              )}
                              ...
                              <div style={{ marginTop: 12, paddingTop: 12, borderTop: "1px solid var(--border-subtle)" }}>
                                <div style={{ display: "flex", gap: 6, marginBottom: 12 }} role="group" aria-label="Permission presets">
                                  {PRESETS.map((p) => (
                                    <button
                                      ...
                                      className="hoverable"
                                      title={p.desc}
                                      style={{ background: "var(--bg-raised)", border: "1px solid var(--border-subtle)", color: "var(--text-secondary)", borderRadius: "var(--radius-sm)", padding: "4px 10px", fontSize: "var(--text-sm)", fontWeight: 500, cursor: "pointer" }}
                                    >
                                      {p.name}
                                    </button>
                                  ))}
                                ...
                                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(210px, 1fr))", gap: 14 }}>
                                  {GRANT_GROUPS.map((group) => (
                                    <div key={group.title}>
                                      <div style={{ fontSize: "var(--text-sm)", fontWeight: 700, color: "var(--text-tertiary)", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 6 }}>
                                        {group.title}
                                      </div>
                                      {group.fields.map(({ key, label, desc }) => (
                                        <label
                                          key={key}
                                          title={desc}
                                          style={{ display: "flex", alignItems: "flex-start", gap: 8, fontSize: "var(--text-sm)", cursor: "pointer", padding: "5px 6px", margin: "0 -6px", borderRadius: "var(--radius-sm)", background: grant[key] ? "rgba(10, 132, 255, 0.08)" : "transparent", color: grant[key] ? "var(--text-primary)" : "var(--text-secondary)" }}
                                        >
                                          ...
                                          <span>
                                            ...
                                            <span style={{ display: "block", fontSize: "var(--text-sm)", color: "var(--text-tertiary)", marginTop: 1 }}>{desc}</span>
                                          </span>
                                        </label>
                                      ))}
                                    </div>
...
function SummaryCard({ icon, tint, color, label, value }: { icon: IconName; tint: string; color: string; label: string; value: number }) {
  return (
    <div style={{ background: "var(--bg-raised)", border: "1px solid var(--border-subtle)", borderRadius: "var(--radius-md)", padding: "12px 16px", display: "flex", alignItems: "center", gap: 12 }}>
      <span style={{ width: 36, height: 36, borderRadius: "var(--radius-md)", background: tint, color, display: "inline-flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
        <Icon name={icon} size={18} />
      </span>
      <span>
        <span style={{ display: "block", fontSize: "var(--text-sm)", color: "var(--text-tertiary)", textTransform: "uppercase", fontWeight: 600 }}>{label}</span>
        <span style={{ display: "block", fontSize: "var(--text-lg)", fontWeight: 700, color: "var(--text-primary)" }}>{value}</span>
      </span>
    </div>
  );
}
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal-card mount-rise" onClick={(e) => e.stopPropagation()} style={{ width: 440, maxWidth: "90vw", padding: 26 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
          <h3 style={{ fontSize: "var(--text-lg)", fontWeight: 700, margin: 0 }}>New pairing</h3>
        ...
        {!ready ? (
          <p style={{ fontSize: "var(--text-sm)", color: "var(--warning)", background: "rgba(255, 159, 10, 0.12)", padding: "12px 14px", borderRadius: "var(--radius-sm)", margin: 0, lineHeight: 1.5 }}>
            Pairing needs at least one executive and one assistant account. Add them under Users first.
          </p>
        ) : (
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div>
              <label htmlFor="pairing-executive" style={{ display: "block", fontSize: "var(--text-sm)", fontWeight: 600, marginBottom: 6 }}>Executive</label>
              <select className="input-standard select" id="pairing-executive" value={executiveId} onChange={(e) => setExecutiveId(e.target.value)}  style={{ width: "100%" }}>
                {executives.map((u) => (
            ...
            <div>
              <label htmlFor="pairing-assistant" style={{ display: "block", fontSize: "var(--text-sm)", fontWeight: 600, marginBottom: 6 }}>Assistant</label>
              <select className="input-standard select" id="pairing-assistant" value={assistantId} onChange={(e) => setAssistantId(e.target.value)}  style={{ width: "100%" }}>
                {assistants.map((u) => (
                  <option key={u.id} value={u.id}>{u.display_name} ({u.email})</option>
                ))}
              </select>
