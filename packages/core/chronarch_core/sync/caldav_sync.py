"""Backfill + calendar discovery for a CalDAV account (BR-CAL-003).

Same windowed-backfill + prune contract as the Google/Microsoft sync
modules: discovers calendar collections, backfills [now-90d, now+365d],
deletes cached rows missing upstream (except unwritten local rows).
CalDAV has no incremental sync-token in this implementation, so every
reconciliation is a full-window backfill.
"""
...
def _caldav_credentials(account: Account) -> tuple[str, str, str]:
    if not account.caldav_server_url or not account.caldav_username:
        raise ValueError("CalDAV account is missing server URL or username")
    if not account.encrypted_caldav_password:
        raise ValueError("CalDAV account has no stored password")
    password = get_cipher().decrypt(account.encrypted_caldav_password)
    return account.caldav_server_url, account.caldav_username, password


async def sync_caldav_account(session: AsyncSession, account: Account) -> dict:
    server_url, username, password = _caldav_credentials(account)
    connector = CalDAVConnector(server_url, username, password)
